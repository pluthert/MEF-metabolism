...
% now select subset, let's start with 
...
includeRealFlux = 1;    % use real world measures of flux and not just DMEM concentrations
scaleRealFlux = 1;       % scale real world measures to WT glucose in order to not create spurious saturation of internal gene expression constraints
...
%       ACROSS SELECTED GROUPS GENERATING THE MATRIX fluxMaxTab
    ...
    %
        ...
        for sampleNo = 1:size(S,2) % cycle through all the separate experimental samples                               
            ...
            % GLUCOSE OR MEASURED UPTAKE WITH ADDITIONAL OPTION TO SCALE TO
            % WT GLUCOSE. NOTE SPECIFIC FOR MEF's. NEED GENERALISING TO USE
            % WITH OTHER DATA
            ...
            if includeRealFlux == 1
                ...
                if scaleRealFlux == 1
                    for colIndex = 3:5
                        METs_tab{:,colIndex} = METs_tab{:,colIndex}*(METs_tab{1,2}/METs_tab{1,colIndex});
                    end
                end
...
% Now run the extra analyses

% spreadsheetAnalyticsComprehensive20220221;
% compressionDataMake;
% expressionMatrixCompressionMulti3;
